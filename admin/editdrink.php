<?php include('header.php') ;
    ...
    if(isset($_POST['save'])){
        ...
        if(!$errors){
            $sql = "UPDATE `thucuong` SET `tieude`='$tieude',`mota`='$mota',`gia`='$gia',`hinhanh`='$tenhinhanh',`noibat`='$noibat',`active`='$active' where id =$id";
            $res = mysqli_query($conn,$sql);
            if($res == true){
                $_SESSION['themthucuong1']="<div class='text-success mb-3'><b>Chỉnh sửa thức uống thành công</b></div>";
                header('location: showdrink.php');
            }
        }else{
            $_SESSION['themthucuong'] = "<div class='text-danger mb-3 text-center'><b>Chỉnh sửa thức uống thất bại</b></div>";
        }
...
<div class="container show_admin">
    ...
    <h2 class="text-center mt-4" style="background-color:#FFEFD5;">Chỉnh sửa thông tin thức uống</h2>
    <?php 
